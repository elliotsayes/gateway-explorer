...
export const StringComparitor = ({ label, expected, actual }: Props) => {
  if (typeof expected !== "string") {
    return (
      <div className="flex flex-row items-center gap-1">
        ...
        />
        <span>{label}: </span>
        <code className="text-muted-foreground">({actual ?? JSON.stringify(actual)})</code>
      </div>
    )
  }
  ...
  if (expected === actual) {
    return (
      <div className="flex flex-row items-center gap-1">
        ...
        />
        <span>{label}: </span>
        <code className="text-muted-foreground">{actual}</code>
      </div>
    )
  }
  ...
  return (
    <div className="flex flex-row items-center gap-1">
      ...
      />
      <span>{label}: </span>
      <code className="text-muted-foreground">({actual ?? typeof actual})</code>
      <TooltipProvider>
        <Tooltip>
          <TooltipTrigger>
            <InfoIcon
